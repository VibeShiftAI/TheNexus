...
- **Authority (who may waive it):** Praxis, via the task re-point endpoint — POST /api/tasks/a2aace88-46f7-4054-8cba-cac7f1a3f087/workspace with the new path and a reason. Robert, if the re-point is refused.
...
- **Authority (who may waive it):** The QA reviewer, and only through a ruling: contest a defective criterion with "PRAXIS_CRITERION_DISPUTE: <number> — <why, with evidence>". Twice contested without resolution goes to Robert.
...
#### BC-WORKSPACE-BINDING — Your home workspace is `/Volumes/Projects/TheNexus` and `/Volumes/Projects/Praxis`. You MAY work in additional repos under /Volumes/Projects when the task genuinely needs it, and you MUST name every extra repo you touched — absolute path — and why, in your completion report.
