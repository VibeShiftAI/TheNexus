/**
 * Nexus Logo Animated MP4 Exporter
 * 
 * Renders the animated Nexus logo to an MP4 video using @napi-rs/canvas
 * and the bundled ffmpeg from @ffmpeg-installer/ffmpeg.
 * 
 * Usage: node scripts/export-logo-animated.js [size] [duration] [output-path]
 *   size        - Logo dimensions in pixels (default: 512)
 *   duration    - Duration in seconds (default: 5)
 *   output-path - Output file path (default: nexus-logo.mp4)
 * 
 * Examples:
 *   node scripts/export-logo-animated.js
 *   node scripts/export-logo-animated.js 1024 10
 *   node scripts/export-logo-animated.js 512 5 ./assets/logo.mp4
 */
...
const outputPath = process.argv[4] || path.join(__dirname, '..', 'nexus-logo.mp4');
const FPS = 60;
...
function initializeEffects() {
    ...
    const addStroke = (x1, y1, x2, y2, count, jitter) => {
    ...
    // LEFT VERTICAL
    addStroke(nLeft, nBottom, nLeft, nTop, 3, 0.005);
    addStroke(nLeft - size * 0.025, nBottom, nLeft - size * 0.025, nTop, 1, 0.015);
    addStroke(nLeft + size * 0.015, nBottom, nLeft + size * 0.015, nTop, 1, 0.015);

    // DIAGONAL
    addStroke(nLeft, nTop, nRight, nBottom, 3, 0.005);
    addStroke(nLeft, nTop + size * 0.02, nRight, nBottom + size * 0.02, 1, 0.005);
    addStroke(nLeft + size * 0.02, nTop, nRight - size * 0.02, nBottom, 1, 0.01);

    // RIGHT VERTICAL
    addStroke(nRight, nTop, nRight, nBottom, 3, 0.005);
    addStroke(nRight + size * 0.025, nTop, nRight + size * 0.025, nBottom, 1, 0.015);
    addStroke(nRight - size * 0.015, nTop, nRight - size * 0.015, nBottom, 1, 0.015);
...
// --- Main ---
async function main() {
    console.log(`🎬 Nexus Logo Animated Exporter`);
    ...
    // Spawn ffmpeg, piping raw RGBA frames via stdin
    const ffmpeg = spawn(ffmpegPath, [
        '-y',                           // Overwrite output
        '-f', 'rawvideo',               // Input format: raw pixels
        '-vcodec', 'rawvideo',
        '-pix_fmt', 'rgba',             // Pixel format
        '-s', `${size}x${size}`,        // Frame size
        '-r', `${FPS}`,                 // Frame rate
        '-i', '-',                      // Read from stdin
        '-c:v', 'libx264',             // H.264 codec
        '-pix_fmt', 'yuv420p',         // Compatible pixel format for output
    ...
    for (let frame = 0; frame < TOTAL_FRAMES; frame++) {
        ...
        if (frame % FPS === 0 || frame === TOTAL_FRAMES - 1) {
            ...
            process.stdout.write(`\r   Rendering: ${pct}% (frame ${frame + 1}/${TOTAL_FRAMES})`);
        }
    }
    ...
    await new Promise((resolve, reject) => {
        ffmpeg.on('close', (code) => {
            if (code === 0) {
                ...
                console.error(stderrData.slice(-500));
    ...
    console.log('\n');
    console.log(`✅ Animated logo exported successfully to: ${outputPath}`);
    ...
    console.log(`   File size: ${fileSizeKB > 1024 ? fileSizeMB + ' MB' : fileSizeKB + ' KB'}`);
}
