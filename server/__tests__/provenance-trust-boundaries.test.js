...
// capture tool callbacks, then invoke the two read tools QA flagged
// (nexus_get_board_state, nexus_get_task) and assert external-tier commands
// are withheld.
//
// Since P1-15 these two tools no longer read `../../db` directly — they
// delegate to board-ops (services/praxis-mind-mcp/lib/board-ops.js), which
// resolves a caller via lib/auth (PRAXIS_MIND_KEY -> ~/.praxis-mind/keys.json)
// and reads through lib/backends (HTTP to the Nexus API). This harness
// mirrors the one in mcp-stateless-conformance.test.js / mcp-board-governance
// .test.js: a real authenticated caller against a scratch 0600 keys.json, and
// lib/backends/ledger/ratelimit/lock-health/config mocked underneath it.
describe('server/mcp.js nexus read tools gate external-tier payloads', () => {
  const GATE_KEY = 'gate-test-key';
  // nexus_get_task governs as OPS.TASK_STATUS (needs nexus.task_status) and
  // nexus_get_board_state as OPS.BOARD_STATE (needs both nexus.projects_list
  // and nexus.tasks_read) — see board-ops.js getTask/boardState and GATES.
  const READER = {
    identity: 'gate-test-reader',
    namespace: 'coding-agents-gate-test',
    privileges: ['nexus.tasks_read', 'nexus.task_status', 'nexus.projects_list'],
  };

  let gateTmp;

  beforeAll(() => {
    gateTmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mcp-gate-'));
    fs.writeFileSync(
      path.join(gateTmp, 'keys.json'),
      JSON.stringify({ keys: { [GATE_KEY]: READER } }),
      { mode: 0o600 },
    );
  });
  ...
  // `backends` stands in for lib/backends — the HTTP-to-Nexus-API seam
  // board-ops reads through. `applyGate` (default: identity) lets the
  // board-state test simulate what the real GET /api/board-state route
  // does (server/routes/dashboard.js applies guardDispatchPayload per task
  // there, but board-ops.boardState itself does not — see that test below).
  function loadMcpHandlers({ nexusTaskById, nexusBoardState }) {
    process.env.PRAXIS_MIND_KEY = GATE_KEY;
    const handlers = {};
    ...
    jest.doMock(`${SVC}/lib/backends`, () => ({
      nexusTaskById: nexusTaskById || jest.fn(async () => { throw new Error('nexusTaskById not stubbed'); }),
      nexusBoardState: nexusBoardState || jest.fn(async () => { throw new Error('nexusBoardState not stubbed'); }),
    }));
    jest.doMock(`${SVC}/lib/log`, () => ({ log: () => {} }));
    jest.doMock(`${SVC}/lib/config`, () => ({
      TRANSITION_LOG: path.join(gateTmp, 'transitions.jsonl'),
      LEDGER_DB: ':memory:',
      KEYS_FILE: path.join(gateTmp, 'keys.json'),
      VAULT: gateTmp,
    }));
    ...
    // ../../db is untouched by the two tools under test here, but mcp.js
    // requires it at module scope (the `projects://list` resource, other
    // tools) — stub it so require('../mcp') doesn't need a real db file.
    jest.doMock('../../db', () => ({ getProjects: jest.fn(async () => []) }));
    jest.isolateModules(() => { require('../mcp'); });
    return handlers;
  }

  it('nexus_get_task withholds an external-tier task\'s commands and wraps its prompt', async () => {
    // nexus_get_task applies guardDispatchPayload itself, after boardOps
    // .getTask returns (server/mcp.js ~line 576) — the gate fires
    // regardless of which backend served the task, so a directly-mocked
    // nexusTaskById is enough to exercise it.
    const handlers = loadMcpHandlers({ nexusTaskById: jest.fn(async () => extTask) });
  ...
  it('nexus_get_board_state withholds external-tier commands but leaves agent-tier untouched', async () => {
    // KNOWN GAP, confirmed by reading the source (not asserted here — this
    // comment documents the finding for the ticket): board-ops.boardState
    // (services/praxis-mind-mcp/lib/board-ops.js ~line 172-174) returns
    // backends.nexusBoardState(...) with NO guardDispatchPayload applied.
    // The gate for board state lives only in server/routes/dashboard.js's
    // GET /api/board-state handler (~line 31-34), which calls
    // `db.getBoardState` directly and maps guardDispatchPayload over the
    // result before responding. In production that route IS what
    // lib/backends.nexusBoardState calls over HTTP (see
    // services/praxis-mind-mcp/lib/backends.js ~line 164-168), so the gate
    // does fire end-to-end for a real spawn. But nexus_get_board_state
    // itself (server/mcp.js) never calls guardDispatchPayload — it trusts
    // whatever lib/backends.nexusBoardState returns. Mocking lib/backends
    // directly, as this suite must to stay hermetic, bypasses the HTTP hop
    // and therefore the gate; a mock that returned raw extTask/agentTask
    // here would pass today but would NOT catch a regression where the
    // dashboard route's gating breaks. So this mock reproduces exactly what
    // GET /api/board-state does (guardDispatchPayload mapped per task) to
    // keep the assertions meaningful; it does not exercise the boardOps
    // pass-through gap.
    const { guardDispatchPayload } = require('../lib/provenance');
    const nexusBoardState = jest.fn(async () => [{
      id: 'proj-1',
      name: 'TheNexus',
      tasks: [extTask, agentTask].map((t) => (
        t.antigravity_payload ? { ...t, antigravity_payload: guardDispatchPayload(t) } : t
      )),
    }]);
    const handlers = loadMcpHandlers({ nexusBoardState });
