...
/**
 * Render one section's entries, collapsing dated series into a single
 * ranged line that states the count, the date range and the filename
 * pattern, so any member can still be constructed by name.
 */
function pushSectionEntries(lines, section, files) {
  ...
  for (const f of files) {
    ...
    if (g) {
      ...
      for (const recent of keep ? g.members.slice(-keep) : []) {
        lines.push(`  - newest: [\`${recent.name}\`](${section}/${encodeFilename(recent.file)})`);
      }
      continue;
    }
...
function regenerateMemoryIndex() {
  // Header kept to two lines: Claude Code truncates MEMORY.md's loaded
  // context at 200 lines (`strings` on the installed binary shows
  // `Une=200` behind "lines after ${Une} will be truncated"), and the
  // memories/ section below already needs nearly the whole budget.
  const lines = [
    '# MEMORY — shared-mind vault index',
    `> Auto-generated by the vault watcher — do not edit by hand, it is overwritten on every vault change. Generated: ${new Date().toISOString()}`,
  ];

  // memories/ keeps per-file lines (dated series collapse; the rest are
  // individually addressable, per this task's scope) — everything else
  // below has no per-file acceptance criterion, and enumerating it was
  // the largest remaining contributor to exceeding Claude Code's
  // 200-line/25,000-byte MEMORY.md truncation ceiling, so it collapses to
  // a single pointer line with a count.
  {
    const dir = path.join(VAULT, 'memories');
    const files = listMarkdown(dir);
    if (files.length > 0) {
      lines.push('## memories/');
      pushSectionEntries(lines, 'memories', files);
      lines.push('');
    }
  }
  ...
  for (const section of ['projects', 'workflows', 'incidents']) {
    ...
    lines.push(`- \`${section}/\` — ${files.length} file${files.length === 1 ? '' : 's'}, browse the directory directly`);
  }

  // Skills — one-line summaries and trigger hints for every skill already
  // live in SKILLS.md; MEMORY.md only needs a pointer plus a count so a
  // session knows the library exists without re-listing every filename.
  const skillsRoot = path.join(VAULT, 'skills');
